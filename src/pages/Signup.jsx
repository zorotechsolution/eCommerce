      ...
      <section className="bg-slate-50 flex justify-center items-center min-h-[90vh] py-10 px-5 font-sans relative overflow-hidden">
        ...
        <div className="w-full max-w-lg bg-white/70 backdrop-blur-xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl p-8 sm:p-10 relative z-10">
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-black text-[rgb(7,81,89)] tracking-tight mb-2">{t('createYourAccount')}</h1>
            <p className="text-sm text-slate-500 font-medium">{t('signupSubtitle')}</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              <div className="flex-1">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-2 mb-1 block">{t('email')}</label>
              ...
              <div className="flex-1">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-2 mb-1 block">{t('phone')}</label>
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              <div className="flex-1">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-2 mb-1 block">{t('confirmPassword')}</label>
            ...
            <div className="px-1 mt-2">
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-xs text-slate-500 font-medium group-hover:text-slate-800 transition-colors">
                  {t('lang') === 'ta' ? 'விதிமுறைகளை ஒப்புக்கொள்கிறேன்' : 'I agree to the Terms & Conditions'}
                </span>
              </label>
            </div>
            ...
            >
              {loading ? t('signingIn') : t('createAccountBtn')}
            </button>
          </form>
